...
namespace Mixture
{
    std::unordered_map<std::string, ImFont*> ImGuiFonts::s_Fonts = std::unordered_map<std::string, ImFont*>();

    void ImGuiFonts::Add(const std::string& name, ImFont* font)
    {
        s_Fonts[name] = font;
    }

    ImFont* ImGuiFonts::Get(const std::string& name)
    {
        return s_Fonts[name];
    }

    void SetMixtureTheme()
    {
        ...
        // Docking
        colors[ImGuiCol_DockingPreview] = ImGui::ColorConvertU32ToFloat4(Colors::Theme::titlebar);
        colors[ImGuiCol_DockingEmptyBg] = ImGui::ColorConvertU32ToFloat4(Colors::Theme::titlebar);
